...
const AdminCourses: React.FC = () => {
  ...
  const canSubmit = useMemo(() => {
    return name.trim().length > 0 && typeof remain === 'number' && remain > 0 && typeof basket === 'number' && basket > 0;
  }, [name, remain, basket]);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(txt || "과목 등록 실패");
      }
      setModalMsg("과목 등록 성공");
      setModalOpen(true);
      setName(""); setRemain(""); setBasket(""); setTime("");
      await fetchCourses();
    } catch (e: any) {
      setModalMsg(e?.message || "과목 등록 실패");
      setModalOpen(true);
    }
  };
  ...
  const handleDelete = async (courseId: AdminCourse["id"]) => {
    if (!courseId) {
      setModalMsg("유효하지 않은 과목 ID입니다.");
      setModalOpen(true);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const txt = await res.text().catch(() => "");
        throw new Error(txt || "과목 삭제 실패");
      }
      setModalMsg("과목 삭제 성공");
      setModalOpen(true);
      await fetchCourses();
    } catch (e: any) {
      setModalMsg(e?.message || "과목 삭제 실패");
      setModalOpen(true);
    }
  };
    ...
    <div style={{ padding: "96px 20px 28px", maxWidth: 1120, margin: "0 auto" }}>
      ...
      <form onSubmit={handleAdd} style={{
        ...
        <button type="submit" disabled={!canSubmit} style={{
          padding: "10px 12px", background: canSubmit ? "#2563eb" : "#cbd5e1", color: "#fff",
            ...
            <div key={String(c.id) + c.name}
              ...
              <div style={{ textAlign: "center" }}>
                <button onClick={() => handleDelete(c.id)} style={{
                  padding: "6px 10px", background: "#ef4444", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer"
                }}>삭제</button>
              </div>
            </div>
          ))
